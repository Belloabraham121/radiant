...
import { formatChainAddress, getEvmDefaultChainId } from "@/lib/chain-meta";
import {
  connectInjectedEvm,
  parseDecimalToAtomic,
  sendInjectedEvmTransfer,
} from "@/lib/personal-wallet";

type Step = "connect" | "amount" | "confirm" | "done";

type EvmDepositDialogProps = {
  open: boolean;
...
export function EvmDepositDialog({
  open,
  onOpenChange,
  agentAddress,
  onSuccess,
}: EvmDepositDialogProps) {
  ...
  const agentShort = formatChainAddress("ethereum", agentAddress);
  const evmChainId = getEvmDefaultChainId();
  ...
  const connect = async () => {
    ...
    setConfirming(true);
    try {
      const address = await connectInjectedEvm();
  ...
  const submit = async () => {
    ...
    setConfirming(true);
    try {
      const wei = parseDecimalToAtomic(amount, 18);
      if (!wei) throw new Error("Enter a valid ETH amount.");

      const hash = await sendInjectedEvmTransfer({
        from: fromAddress,
        to: agentAddress,
        amountWei: wei,
        chainId: evmChainId,
      ...
      >
        <DialogHeader className="border-b-2 border-[var(--hero-ink)]/10 px-6 py-5">
          <DialogTitle className="font-heading text-xl font-extrabold tracking-tight">
            {step === "connect" && "Connect EVM wallet"}
            {step === "amount" && "Deposit ETH"}
            {step === "confirm" && "Confirm deposit"}
            {step === "done" && "Deposit sent"}
          </DialogTitle>
          <p className="text-sm font-medium text-[var(--hero-ink)]/55">
            Brave Wallet, MetaMask, or any injected EVM wallet on chain {evmChainId}.
          </p>
        </DialogHeader>
        ...
        <div className="px-6 py-5">
          ...
          {step === "amount" && fromAddress && (
            <div className="flex flex-col gap-4">
              <p className="font-mono text-xs text-[var(--hero-ink)]/55">
                From {formatChainAddress("ethereum", fromAddress)}
              </p>
              ...
              <button
                type="button"
                disabled={!parseDecimalToAtomic(amount, 18)}
          ...
          {step === "confirm" && (
            <div className="flex flex-col gap-4">
              <p className="font-heading text-2xl font-extrabold">{amount} ETH</p>
              {error && (
                <p className="rounded-xl border-2 border-[var(--hero-coral)]/30 bg-[var(--hero-coral)]/10 px-3 py-2 text-xs font-semibold text-[var(--hero-coral)]">
                  {error}
                </p>
          ...
          {step === "done" && (
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              {txHash && (
                <p className="break-all font-mono text-xs text-[var(--hero-ink)]/60">{txHash}</p>
              )}
