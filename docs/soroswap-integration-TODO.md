...
| Status | Task | Owner | Path |
| ------ | ---- | ----- | ---- |
| [ ] | `resolveSoroswapQuoteForExecute({ quoteId, routeId, snapshotParams })` | [Backend] | `soroswap-quote.service.ts` |
| [ ] | Re-quote if expired at approval time (mirror DeepBook / Li-Fi) | [Backend] | ↑ |
| [ ] | `executeSoroswapSwap(privyUserId, input)` — build → sign → submit | [Backend] | `backend/src/services/defi/soroswap/soroswap-execute.service.ts` |
| [ ] | Sign via `signStellarTransaction` (Privy `rawSign`) | [Backend] | `stellar-signing.service.ts` |
| [ ] | Submit via Horizon or Soroban RPC per tx type | [Backend] | `stellar-transaction.service.ts` |
| [ ] | Return `{ hash, ledger, … }` compatible with agent execute result | [Backend] | ↑ |
| [ ] | Unit tests: execute validation (mock Privy + RPC) | [Tests] | `soroswap-execute.service.test.ts` |

#### 2.8 Status & tracking (optional v1 — at minimum poll until confirmed)

| Status | Task | Owner | Path |
| ------ | ---- | ----- | ---- |
| [ ] | `getSoroswapSwapStatus(txHash)` — Horizon tx + Soroban result | [Backend] | `backend/src/services/defi/soroswap/soroswap-status.service.ts` |
| [ ] | Map: success, failed, pending | [Backend] | `soroswap-normalize.ts` |
| [ ] | Optional Inngest `soroswap-track-swap` for long confirmation | [Backend] | `inngest/functions/soroswap-track-*.ts` |
| [ ] | Unit tests: status mapping | [Tests] | `soroswap-status.test.ts` |
