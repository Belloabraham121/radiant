...
When you create a Radiant account — Google, GitHub, or email — **your agent gets its own Sui wallet automatically**. You do not connect a wallet to sign up. **[Privy](https://docs.privy.io/) generates the agent wallet** on signup: it provisions an embedded Sui wallet for your agent, ties it to your account, and keeps signing keys in Privy's secure enclave. You hold the keys; Radiant never does. The backend verifies your Privy session and resolves that agent wallet address before any onchain action runs.
...
│   Auth (Google / GitHub / email via Privy)  │
│   Agent wallet (Privy generates on signup)  │
...
│          Hono HTTP server                 │
│     Privy auth + agent wallet lookup      │
│                                           │
│  ┌─────────────────────────────────────┐  │
│  │          Claude API                 │  │
│  │          (tool use mode)            │  │
...
| Auth & wallet     | Privy (auth + **generates** embedded Sui agent wallet on signup); `@mysten/dapp-kit` for optional deposits |
| Backend           | TypeScript, Express/Hono (Arcane-style layered API) |
...
The UI prototype lives in `client/` today. Backend (`backend/`), shared packages, and Move contracts are scaffolded and ready for implementation. Agent wallets are generated by **Privy** on signup; the backend authenticates via Privy and uses the user's embedded Sui wallet for agent actions.
...
│   │   │   ├── auth/                         # AuthCard (Privy login)
│   │   │   ├── wallet/                       # Privy provider, agent wallet hooks
...
│
├── backend/                                  # Node.js API (Arcane-style layout)
│   ├── README.md                             # Layout, Docker, Privy agent wallet
│   ├── api-ref.md                            # Routes & env checklist
...
│       ├── app.ts                            # App factory (planned)
│       ├── api/                              # HTTP layer
│       │   ├── middleware/                   # auth, correlation-id, errors
│       │   └── routes/v1/
│       │       ├── auth/                     # Privy session
│       │       ├── chat/                     # POST /chat
│       │       ├── build/                    # POST /build
...
│       ├── websocket/                        # Chat stream, deploy progress
│       ├── services/
│       │   ├── auth/                         # Privy verification
│       │   ├── wallet/                       # Agent wallet (Privy-generated)
│       │   ├── agent/                        # Claude, tools, templates
│       │   ├── chains/adapters/              # Sui, EVM, Solana adapters
│       │   ├── memory/                       # Walrus agent memory + credentials
│       │   ├── sandbox/                      # E2B
│       │   ├── walrus/                       # Sites + blobs
│       │   └── deploy/                       # Deploy pipeline
│       ├── infrastructure/postgres/          # Prisma client
│       ├── infrastructure/redis/             # Queues, cache
│       ├── config/
│       ├── shared/
│       ├── types/
│       └── utils/
│
├── apps/                                     # Planned monorepo layout (future)
│   └── web/                                  # Production Next.js (future merge from client/)
│
├── packages/
│   ├── move/                                 # Sui Move contracts (scaffolded)
│   │   ├── registry/
│   │   │   └── sources/                      # Radiant AppRegistry module
│   │   └── templates/
│   │       ├── swap/sources/
│   │       ├── prediction/sources/
│   │       └── escrow/sources/
...
```typescript
// backend/src/services/chains/adapter.ts
...
```typescript
// backend/src/services/chains/adapters/sui.ts
...
```typescript
// backend/src/services/chains/adapters/evm.ts
...
```typescript
// backend/src/services/chains/adapters/solana.ts
...
```typescript
// backend/src/services/chains/registry.ts
...
```typescript
// backend/src/services/memory/agent-memory.ts
...
```typescript
// backend/src/services/memory/credentials.ts
...
```typescript
// backend/src/services/agent/tools.ts
...
Open [http://localhost:3000](http://localhost:3000). Sign up at `/auth` (Privy provisions your agent's Sui wallet), then fund your agent at `/app/settings`.
