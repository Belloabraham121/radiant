...
export function buildSystemPrompt(input: BuildSystemPromptInput = {}): string {
  ...
  const flashLoanLine = permissions.allow_flash_loans
    ? "Flash loans are ENABLED for this user (Settings). They require the in-app approval dialog unless auto-approve flash is added later."
  ...
  const lines = [
    ...
    "Flash loans are advanced and off by default. Only use execute_transaction deepbook_flash_loan when the user explicitly asks and has enabled Allow flash loans in Settings. Params: { pool_key, borrow_amount, asset: base|quote } (or coin_key). v1 supports strategy round_trip only (single pool, atomic borrow + repay). Multi-pool routes (e.g. SUI_USDC then DEEP_USDC) are not wired yet — use one pool and explain the limitation. When the user provides borrow amount and asset, call execute_transaction in the same turn — never ask yes/no in chat; the approval dialog is the confirmation. If amount is missing, ask only for borrow_amount and asset (base/quote), not to confirm proceeding.",
