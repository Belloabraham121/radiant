...
export function buildSystemPrompt(input: BuildSystemPromptInput = {}): string {
  ...
  const lines = [
    ...
    "When the user wants a UI built, updated, or previewed in the artifact panel (landing page, dashboard, swap/DEX interface like Uniswap, form, widget), call generate_app with React source files under src/. You MUST always include src/App.tsx (default export) as the entry that imports components from src/components/*.tsx. This is the preview entry point — not a fixed product template, just how the artifact runtime boots.",
    "After generate_app succeeds, briefly describe what you built — the client opens the artifact panel automatically.",
    "generate_app paths must be relative to src/ or public/ with allowed extensions (.tsx, .ts, .css, .json, .html, .svg). Pass project_id to update an existing project; omit it to create a new one. Use list_session_projects to see apps in this chat before updating. Creating a different app in the same chat (e.g. guessing game then counter app) requires omitting project_id so each app gets its own project.",
    "generate_app chat preview is React in a browser iframe (CDN React + Babel) — NOT Vite and NOT Next.js. It loads every src/**/*.tsx file and starts at src/App.tsx. E2B deploy for custom apps uses a separate Vite scaffold only at publish time. Radiant's own UI is Next.js; user-built apps in chat are portable React until deployed.",
    "Use import from react / react-dom / react-router-dom only — no lucide-react or other npm packages. You may add src/*.css files; do not import .css from TSX (styles are applied automatically). Prefer inline styles or className with Tailwind utility classes for simple UIs.",
    "For swap or DEX UIs, use multiple component files under src/components/ (TokenSelect, SwapForm, etc.) and a thin src/App.tsx that composes them — never omit App.tsx.",
