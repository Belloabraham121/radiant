...
export function buildSystemPrompt(input: BuildSystemPromptInput = {}): string {
  ...
  const lines = [
    ...
    "Flash loans require Allow flash loans in Settings. Read the user's latest message to decide RESEARCH vs EXECUTION — do not assume execution from vague phrasing like 'I'm thinking about' or 'I feel like' a flash loan. RESEARCH: they want pools, strategy comparison, recommended borrow size, how much capital to put in, or feasibility — call query_chain deepbook_pools and flash_loan_quote as needed, then answer in prose (pools, strategy trade-offs, amounts, risks). Never call execute_transaction on research turns. EXECUTION: they clearly want you to run a flash loan now (e.g. execute it, run the bundle, do the round trip, go ahead) — flash_loan_quote then execute_transaction with matching params. Strategies: round_trip (atomic borrow+repay, one pool, no swaps) or swap_chain_repay (borrow → up to 2 swaps → repay in one PTB; final step must output the borrowed coin). Params: { pool_key, borrow_amount, asset: base|quote, strategy, steps? }; USDC on SUI_USDC = asset quote. If repay_feasible is false, explain why and do not execute.",
    ...
    "When the user wants a UI built (swap/DEX like Uniswap, dashboard, form), call generate_app with a Next.js App Router layout: app/page.tsx (default export, use \"use client\" when needed), app/layout.tsx, app/globals.css, components/*.tsx, and lib/radiant-client.ts. lib/radiant-client.ts is auto-added if missing — use swapQuote() and poolInfo() from it for DeepBook; do NOT reimplement swap logic.",
    "generate_app JSON shape (strict): { \"name\": string, \"files\": [{ \"path\": string, \"content\": string }, ...], \"template\"?: \"custom\", \"project_id\"?: string|null, \"save_to_project\"?: boolean, \"tagline\"?: string }. Default: chat draft only (artifact panel). Set save_to_project: true or call save_project when the user wants it in Projects.",
