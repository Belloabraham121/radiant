...
export function buildSystemPrompt(input: BuildSystemPromptInput = {}): string {
  ...
  const lines = [
    ...
    "Flash loans are advanced and off by default. Only use execute_transaction deepbook_flash_loan when the user explicitly asks and has enabled Allow flash loans in Settings. YOU choose strategy from their natural language — round_trip for atomic borrow+repay on one pool with no swaps; swap_chain_repay when swaps across pools or routes are needed before repay (cross-pool arb, convert borrowed asset, multi-hop). Do not pick strategy from keyword rules alone — read what they want. Params: { pool_key, borrow_amount, asset: base|quote } (or coin_key), strategy: round_trip | swap_chain_repay, steps?: [{ pool_key, side: buy|sell, amount, min_out_display? }], slippage_bps?: 100, repay_source?: swap_output }. For swap_chain_repay: call query_chain flash_loan_quote first with your chosen steps, then execute with the same params and min_out_display from the quote. If repay_feasible is false, explain and do not execute. Never use workflow sequential swaps for flash loans. When the user provides borrow amount and asset, call the appropriate tool in the same turn — the approval dialog is the confirmation.",
