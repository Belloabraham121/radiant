...
export function buildSystemPrompt(input: BuildSystemPromptInput = {}): string {
  ...
  const flashLoanLine = permissions.allow_flash_loans
    ? permissions.auto_approve_flash_loans
      ? "Flash loans are ENABLED with auto-approve ON — when the user asks you to run a flash loan (including picking a strategy you already proposed), call flash_loan_quote and execute_transaction in the same turn without asking in chat to confirm pool, asset, or amount. swap_chain_repay bundles that repay from swap output execute automatically; wallet-repay still requires approval."
      : "Flash loans are ENABLED — every flash loan shows the in-app approval dialog unless auto-approve flash loans is turned on. Never ask in chat to confirm execution — call the tool and let the dialog handle it."
  ...
  const lines = [
    ...
    "Flash loans are advanced and off by default. Only use deepbook_flash_loan when the user explicitly asks and has enabled Allow flash loans in Settings. YOU choose strategy from natural language and thread context — round_trip for atomic borrow+repay on one pool with no swaps; swap_chain_repay when swaps across pools are needed before repay. If you previously listed numbered strategies with amounts and the user picks one ('use the second strategy', 'execute it', 'run that'), read your prior message for pool_key, borrow_amount, asset (base|quote), strategy, and swap steps — then call tools in the same turn. Only ask for borrow amount if nowhere in this thread (including your own replies) is a concrete amount. Params: { pool_key, borrow_amount, asset: base|quote } (or matching coin_key), strategy, steps?: [{ pool_key, side: buy|sell, amount }]. pool_key is the borrow pool; USDC on SUI_USDC = asset quote. First swap step must spend the borrowed coin; final step must output the borrowed coin for repay. For swap_chain_repay: flash_loan_quote first, then execute with same params + min_out_display. If repay_feasible is false, explain and do not execute. Never ask in chat to confirm pool/asset/amount when already stated. Never use workflow sequential swaps for flash loans.",
