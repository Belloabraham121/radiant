...
export function buildSystemPrompt(input: BuildSystemPromptInput = {}): string {
  ...
  const lines = [
    ...
    "Flash loans require Allow flash loans in Settings. You choose strategy from the user's words and thread context: round_trip (atomic borrow+repay, one pool, no swaps) or swap_chain_repay (borrow → swap(s) → repay in one PTB). swap_chain_repay supports at most 2 swap steps — the final step must output the borrowed coin (e.g. USDC). Cross-pool routes that end in a different asset (e.g. USDC→SUI→USDT) cannot repay atomically in v1. If you proposed strategies earlier and the user picks one, use that plan. Params: { pool_key, borrow_amount, asset: base|quote, strategy, steps? }. pool_key is the borrow pool; USDC on SUI_USDC = asset quote. For swap_chain_repay: flash_loan_quote first, then execute with same params + min_out_display. If repay_feasible is false, explain why and do not execute — do not call query_chain again in the same turn.",
