...
export function buildLifiBridgeLines(): string[] {
  ...
  return [
    ...
    "cross_chain_swap params: route_id (required — the server resolves the full route from it; do NOT echo back the large lifi_route object) plus the lightweight snapshot fields from the chosen route (from_token, to_token, from_token_symbol, to_token_symbol, from_amount_atomic, to_amount_atomic, from_chain_id, to_chain_id, to_evm_chain_id, bridges, fee_cost_usd, expires_at) — these populate the approval dialog with pay/receive amounts and the quote countdown timer.",
